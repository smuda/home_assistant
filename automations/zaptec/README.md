...
1. **Available current mode** — *how* current is regulated:
...
2. **Eco Mode** — a separate feature layer on top of the mode
...
Repeated stop/start is also flagged by Zaptec
...
when a phase crosses 23 A. It runs while the car is charging or
requesting, or while the fuse-pause helper is on (so resume can be
evaluated). At session start the last change is old, so the first
correct value is written right away.
...
feed is dead, the phase's
non-car load is set to `failsafe_other` (18 A), which floors the car
to 6 A — a meter dropout is never read as zero load (which would
wrongly allow the full 16 A). If even the raw meter is gone, the
same fail-safe applies. The wake-up trigger still watches the raw
...
the P1 phase currents (outlier rejection + short moving average).
They are sensor platforms, not automations, so they go in
...
- **Mode is not visible in HA.** The integration exposes no entity
  for the available-current mode or Eco Mode (custom-components/
  zaptec issue #153). Standard charging + Eco is set in the app.
